...
!(function (e, t) {
	...
	class b {
		...
		table(e) {
			...
			if (n.length === s.length) {
				...
				for (const e of r) i.rows.push(f(e, i.header.length).map((e, t) => ({ text: e, tokens: this.lexer.inline(e), header: !1, align: i.align[t] })));
				return i;
			}
		}
		lheading(e) {
		...
		link(e) {
			...
			if (t) {
				...
				return (
					...
					x(t, { href: n ? n.replace(this.rules.inline.anyPunctuation, '$1') : n, title: s ? s.replace(this.rules.inline.anyPunctuation, '$1') : s }, t[0], this.lexer)
				);
			}
		}
		reflink(e, t) {
	...
	const w = /^ {0,3}((?:-[\t ]*){3,}|(?:_[ \t]*){3,}|(?:\*[ \t]*){3,})(?:\n+|$)/,
		...
		F = u('^comment|^</[a-zA-Z][\\w:-]*\\s*>|^<[a-zA-Z][\\w-]*(?:attribute)*?\\s*/?>|^<\\?[\\s\\S]*?\\?>|^<![a-zA-Z]+\\s[\\s\\S]*?>|^<!\\[CDATA\\[[\\s\\S]*?\\]\\]>')
			.replace('comment', H)
				...
				if (
					...
					if ((s = this.tokenizer.space(e))) (e = e.substring(s.raw.length)), 1 === s.raw.length && t.length > 0 ? (t[t.length - 1].raw += '\n') : t.push(s);
						...
						if (this.state.top && (s = this.tokenizer.paragraph(i)))
							(r = t[t.length - 1]),
								n && 'paragraph' === r?.type
									? ((r.raw += '\n' + s.raw), (r.text += '\n' + s.text), this.inlineQueue.pop(), (this.inlineQueue[this.inlineQueue.length - 1].src = r.text))
									: t.push(s),
								(n = i.length !== e.length),
								(e = e.substring(s.raw.length));
			...
			switch (t) {
				...
				case 'table':
					return function (n) {
						...
						let s = '',
							r = '';
						for (let e = 0; e < n.header.length; e++) r += this.tablecell({ text: n.header[e].text, tokens: n.header[e].tokens, header: !0, align: n.align[e] });
				...
				case 'list':
					return function (n) {
						...
						for (let e = 0; e < n.items.length; e++) {
							...
							(o += this.parser.parse(t.tokens, i)), (l += this.listitem({ type: 'list_item', raw: o, text: o, task: r, checked: !!s, loose: i, tokens: t.tokens }));
						}
