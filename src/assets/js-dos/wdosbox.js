	...
	return function (WDOSBOX) {
		...
		function emsc_ws_client_sound_init(freq) {
			if (Module.directSound !== undefined) {
				var directSound = { ringSize: Module.directSound.ringSize, bufferSize: Module.directSound.bufferSize, buffer: [] };
		...
		function ws_client_network_connected(networkType, address, port) {
			Module.sendMessage('ws-connected', { networkType: networkType, address: UTF8ToString(address), port: port });
		}
		...
		function emsc_extract_bundle_to_fs() {
			...
			Module.libzip_progress = function (file, extracted, count) {
				Module.sendMessage('ws-extract-progress', { index: index, file: file, extracted: extracted, count: count });
			};
