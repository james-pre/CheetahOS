...
export class TaskmanagerComponent implements BaseComponent, OnInit, OnDestroy, AfterViewInit {
	...
	constructor(processIdService: ProcessIDService, runningProcessService: RunningProcessService, notificationService: NotificationService, renderer: Renderer2) {
	...
	hideShowNavMenu(menuName: string): void {
		...
		if (menuName == '') {
			...
			if (menuElmt) menuElmt.style.display = 'none';
	...
	generateLies(): void {
		...
		this.sleepNumber == 0 ? (this.sleepNumber = this.getRandomNums(minNum, maxNum * maxNum * 2)) : this.sleepNumber;

		this.processNumberToSuspend == 0
			? (this.processNumberToSuspend = processes[this.getRandomNums(minNum - 1, processes.length - 1)].getProcessId)
			: this.processNumberToSuspend;
	...
	addTrailingZeros(num: number): number {
		...
		if (num != 0) {
			if (strNum.length == 1) return parseFloat(strNum.padEnd(totalLength, '.1'));
		}
		return num;
	}
	...
	applyDefaultColumnStyle(): void {
		const tableColumns: string[] = [
			...
			TableColumns.STATUS,
			TableColumns.PID,
			TableColumns.PROCESS_NAME,
			TableColumns.CPU,
			TableColumns.MEMORY,
			TableColumns.DISK,
			TableColumns.NETWORK,
			TableColumns.GPU,
			TableColumns.POWER_USAGE,
		];
