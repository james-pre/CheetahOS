...
export class WindowComponent implements OnInit, OnChanges, AfterViewInit, OnDestroy {
	...
	constructor(
		...
		this._restoreOrMinSub = this._runningProcessService.restoreOrMinimizeWindowNotify.subscribe(p => {
			this.restoreHiddenWindow(p);
		});
		this._focusOnNextProcessSub = this._runningProcessService.focusOnNextProcessNotify.subscribe(() => {
			this.setNextWindowToFocus();
		});
		this._focusOnCurrentProcessSub = this._runningProcessService.focusOnCurrentProcessNotify.subscribe(p => {
			this.setFocusOnWindow(p);
		});
		this._focusOutOtherProcessSub = this._runningProcessService.focusOutOtherProcessNotify.subscribe(p => {
	...
	setWindowToFocusById(pid: number): void {
		...
		if (windowState !== undefined) {
			if (windowState.pid == pid && windowState.z_index != z_index) {
				if (!z_index ? (z_index = 1) : (z_index = z_index + 1)) this._stateManagmentService.addState(this.z_index, z_index);
