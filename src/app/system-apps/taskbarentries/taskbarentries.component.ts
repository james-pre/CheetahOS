...
export class TaskBarEntriesComponent implements AfterViewInit, OnDestroy {
	...
	constructor(processIdService: ProcessIDService, runningProcessService: RunningProcessService, menuService: MenuService, triggerProcessService: TriggerProcessService) {
		...
		this._openApplicationFromTaskbarSub = this._menuService.openApplicationFromTaskBar.subscribe(p => {
			this.openApplication(p);
		});
		this._closeApplicationsFromTaskbarSub = this._menuService.closeApplicationFromTaskBar.subscribe(p => {
	...
	pinIconToTaskBarList(file: FileInfo): void {
		if (!this.pinToTaskBarList.some(x => x.getOpensWith === file.getOpensWith)) this.pinToTaskBarList.push(file);
	...
	onMouseEnter(appName: string, iconPath: string, caller: string): void {
		...
		if (this._runningProcessService.isProcessRunning(appName)) this._runningProcessService.showPreviewWindowNotify.next(data);
	}
