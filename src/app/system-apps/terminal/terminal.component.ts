...
export class TerminalComponent implements BaseComponent, OnInit, AfterViewInit, OnDestroy {
	...
	populateWelecomeMessageField(): void {
		...
		const interval = setInterval(
			msg => {
				...
				if (this.msgPosCounter == msg.length - 1) clearInterval(interval);
	...
	removeCurrentDir(arg0: string): string {
		...
		if (this.isWhiteSpaceAtTheEnd) return arg0;

		if (arg0.includes('/')) {
			const argSplit = arg0.split('/').filter(x => x !== '');
			const res: string[] = [];

			if (argSplit.length === 1) return `${argSplit[0]}/`;
			else {
				argSplit.pop();
	...
	getCommandHistory(direction: string): void {
		...
		if (this.commandHistory.length > 0) {
			if (direction === 'backward') {
				...
				currPtrIndex = this.prevPtrIndex === this.commandHistory.length ? this.commandHistory.length : this.prevPtrIndex + 1;
			}
	...
	async processCommand(terminalCmd: TerminalCommand, key = ''): Promise<void> {
		...
		if (this.isValidCommand(rootCmd)) {
			...
			if (rootCmd == 'cd') {
				...
				if (result.type === str || result.type === strArr) terminalCmd.setResponseCode = this.Success;
